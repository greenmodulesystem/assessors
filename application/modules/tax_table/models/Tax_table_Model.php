...
class Tax_table_Model extends CI_Model
{
    ...
    public function get_tax_table_m()
    {
        $this->db->select(
            '*'
        );
        $this->db->from('tbl_tax_manufacturer');
        $this->db->order_by('ID', 'asc');
        $result = $this->db->get()->result();
        return $result;
    }

    public function get_tax_table_d()
    {
        $this->db->select(
            '*'
        );
        $this->db->order_by('ID', 'asc');
        $this->db->from('tbl_tax_dealer');
        $result = $this->db->get()->result();
        return $result;
    }

    public function get_tax_table_s()
    {
        $this->db->select(
            '*'
        );
        $this->db->order_by('ID', 'asc');
        $this->db->from('tbl_tax_service');
        $result = $this->db->get()->result();
        return $result;
    }

    public function get_tax_table_r()
    {
        $this->db->select(
            '*'
        );
        // $this->db->order_by('ID', 'asc');
        $this->db->from('tbl_tax_retailer');
        $result = $this->db->get()->row();
        return $result;
    }

    public function get_tax_table_other()
    {
        $this->db->select(
            '*'
        );
        $this->db->from('tbl_tax_other');
        $result = $this->db->get()->result();
        return $result;
    }

    public function get_tax_table_fixed()
    {
        $this->db->select(
            '*'
        );
