$(document).ready(function () {
  $("#Save,#Save_gross").on("click", function () {
    ...
    Object.keys(blines).forEach(function (key) {
      ...
      var ess_val =
        $("#ess" + key).val() == null || $("#ess" + key).val() == 0
          ? null
          : $("#ess" + key).val();
      var non_val =
        $("#non" + key).val() == null || $("#non" + key).val() == 0
          ? null
          : $("#non" + key).val();
      ...
      var mp_val =
        $("#mp" + key).val() == null
          ? $("#mp_fee  " + key).val()
          : $("#mp" + key).val();
      mp_ID.push(mp_val); //added 5824 alob
      var exempt = $("#xmp" + key).is(":checked") ? 1 : 0;
      Exempted.push(exempt);
    });

    var truck = $("#Trucking_Fee").val();
    truck =
      truck < 500
        ? truck
        : truck >= 500 && truck < 600
        ...
        if (missingValues()) {} else {
          $.ajax({
              ...
              data: {
                  ...
                  Beach: $('#Beach_fee').val(),
                  BrgyFee: $('#Brgy_fee').val(),
                  // Solid_waste_ID: $('#Beach_fee').val(),
  ...
  $("#Trucking").on("change", function () {
    ...
    if ($(this).is(":checked")) {
      ...
      // var truck = $('#Trucking_Fee').val() > 500 ? : ;
      //   alert();
    } else {
  ...
  $("#BrgyF").on("change", function () {
    if ($(this).is(":checked")) {
      $("#Brgy_fee").removeAttr("disabled");
    } else {
      $("#Brgy_fee").prop("disabled", true);
    }
  });   
